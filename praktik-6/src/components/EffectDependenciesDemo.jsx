...
const EffectDependenciesDemo = () => {
  ...
  // Effect 2: Empty dependencies - run sekali setelah mount [cite: 373, 374, 393]
  useEffect(() => {
  ...
  // Effect 3: Dependency count - run ketika count berubah [cite: 417, 421, 403]
  useEffect(() => {
  ...
  // Effect 4: Dependency name - run ketika name berubah [cite: 422, 425]
  useEffect(() => {
    if (name) { // Hanya log jika name tidak kosong
  ...
  // Effect 6: Window resize event listener dengan cleanup [cite: 435, 454]
  useEffect(() => {
    const handleResize = () => {
  ...
  // Effect 7: Timer dengan cleanup [cite: 467, 478]
  useEffect(() => {
