  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setLoading(true);
        setError(null);
        // Simulasi API call dengan timeout 1 detik
    ...
    fetchUsers();
  }, []); // Empty dependencies hanya sekali saat mount [cite: 169]

  // Effect untuk fetching user detail berdasarkan userId
  useEffect(() => {
    const fetchUserDetail = async () => {
      if (!userId) return; // Perbaikan: `!userId` bukan `luserId`

      try {
        setLoading(true);
        setError(null);
        // Simulasi API call dengan timeout 0.5 detik
        await new Promise(resolve => setTimeout(resolve, 500));

        // Data dummy user detail
        const dummyUserDetail = {
          id: userId,
          name: `User ${userId}`, // Perbaikan: Gunakan backticks untuk template literal
    ...
    fetchUserDetail();
  }, [userId]); // Dependency: userId re-fetch ketika userId berubah [cite: 220]
